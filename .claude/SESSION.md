...
- **OPERATOR-OVERRIDE list (highest precedence):** for a SMALL set of items (configs/certs) the operator supplies the exact value+target; applied verbatim, NOT map-rewritten/FAT1-reused. Precedence: override → reuse-FAT1 (certs/keys) → map-rewrite (text) → copy-as-is (binary). LEVEL-2 audit will take `OVERRIDE_PATHS` and mark those `OPERATOR-OVERRIDE (expected to differ)` not DRIFT (wire into audit_env.sh once the path list is known).
