...
- **Liveness heartbeat (`progress`/`progress_done`/`tick`/`count_filter` in `common.sh`)** — long find-walks and per-file loops on deep trees run silent for minutes and look hung. Wrap heavy loops with `tick "$n" "label: $n"` (delimiter-agnostic; safe inside `-print0`/`read -d ''` loops) and walks with `count_filter` (NEWLINE streams only — never insert into a `-print0` pipeline). All are **TTY-gated** (`[ -t 2 ]`) and write only to stderr, so they are a complete no-op under redirect/pipe/cron and NEVER pollute stdout, `LOG_FILE`, reports, or data files. `PROGRESS_EVERY=0` disables. `csv_read_3col` ticks per row automatically (label via `CSV_PROGRESS_LABEL`). Standalone scripts that don't source `common.sh` (`audit_env.sh`, `fix_dir_mtimes.sh`) carry an inline copy — keep it in sync with the canonical one.
